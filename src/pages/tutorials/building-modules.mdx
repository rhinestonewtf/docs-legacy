...
2. Duplicate the file `src/validators/ValidatorTemplate.sol` and rename it to `SimpleValidator.sol` and the contract to `SimpleValidator`.
...
2. Duplicate the file `src/executors/ExecutorTemplate.sol` and rename it to `SimpleExecutor.sol` and the contract to `SimpleExecutor`.
...
and add the following code before the first function in your contract:
...
4. Remove the `executeAction` function (it is not needed anymore) and then add the following functions:
...
Finally, add the following event and errors to the bottom of the contract:

```solidity
event DCAExecuted(address account, uint256 scheduleIndex);

error InvalidTokenAddress();
error InvalidRecipientAddress();
error InvalidAmount(uint256 amount);
error ScheduleNotDueYet(address account, uint256 scheduleIndex);
```
