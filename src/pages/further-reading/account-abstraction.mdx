...
We can’t say it better than Vitalik himself – [The Three Transitions]()
...
-   Bundler – A node (block builder) that can handle UserOperations, create a valid an EntryPoint.handleOps() transaction, and add it to the block while it is still valid.
-   Paymaster – A contract that can define specific gas policies for specific user operations.

## Account abstraction vs. MPC

MPC ≠ Account abstraction. See this article by Lit Protocol, a distributed key management platform.
