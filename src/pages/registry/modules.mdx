...
Modules are smart contracts that hold self-contained features for smart accounts. These modules and their deployment metadata are stored on the Registry, allowing others to attest to them in order for users to find out whether a module is secure or not.

## Module Registration

Modules can be registered on the Registry in two different ways: during and after deployment.
