...
# Tutorial 2: Turn an existing account into an Omni Account

In this tutorial, we will walk through turning an existing smart account into an Omni Account. For the sake of this tutorial, we will still create the smart account but the core part is how to create intents when the account is already deployed on the target chain and has the modules installed. In that case, we can use the more efficient executor flow rather than needing to rely on UserOperations.
...
Next, we will create the clients required to interact with the target chain.
...
Now, we will deploy the account on the target chain. As mentioned above, this is for the reproducability of this tutorial but in most real cases, this account would already be deployed for some reason, such as because of following the previous tutorial.
