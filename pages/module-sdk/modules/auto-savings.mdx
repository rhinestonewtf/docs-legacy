...
Using Auto Savings requires an offchain service like [Rhinestone Automations](/automations). The reason for this is that the users' account is not necessarily called when receiving a token. Further, if a swap is required, we need to calculate the correct parameters for the swap depending on the current price ratio.
