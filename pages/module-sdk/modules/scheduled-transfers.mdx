...
## `getExecuteScheduledTranferAction`

The `getExecuteScheduledTranferAction` function is used to create an execution for execute scheduled transfer. This function returns an execution object.

```typescript
import { getExecuteScheduledTranferAction } from "@rhinestone/module-sdk";

const executeScheduledTranferAction = getExecuteScheduledTranferAction({
  jobId: 1
});
```
