...
- Supports EIP-7702 for chain abstracting EOAs. Data structures and interfaces natively support The Compact, an open-source escrow contract for cross-chain intents, enabling EIP-7702 support.
- Any arbitrary calldata can be appended to the cross-chain intent via injected execution. This enables destination chain userops or any other smart contract interaction to be bundled into one atomic cross-chain intent. 
...
- Orchestrator: An offchain entity that tracks ongoing intents to ensure locked funds cannot be double spent.
