...
contract IntegrationTestExample is RhinestoneModuleKit, Test {
    ...
    function setUp() public {
        ...
        instance = makeRhinestoneAccount("IntegrationTestExample");
...
If you want more custom control over the initial Modules on your account instance, you can use the `makeRhinestoneAccount(bytes32 salt, ERC7579BootstrapConfig[] memory validators, ERC7579BootstrapConfig[] memory executors, ERC7579BootstrapConfig memory hook, ERC7579BootstrapConfig memory fallBack)` function. This requires you to import `ERC7579BootstrapConfig` from `"modulekit/external/ERC7579.sol";` For example:
...
function _setupAccount() public {
    ...
    instance = makeRhinestoneAccount("mainAccount", validators, executors, hook, fallBack);
}
```
...
You can also use the imported `TYPE_[MODULE_TYPE_NAME]` to make it easier to pass the `moduleTypeId` argument (eg TYPE_VALIDATOR).
