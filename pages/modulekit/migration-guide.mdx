...
The module bases repository has been integrated within module-kit and is no longer required as a dependency. You should:
...
Helper changes include:
- All account helpers consolidated in `Helpers.sol`
- `ecdsaSign` moved to `src/test/utils/ECDSA.sol`
...
### Changed Interface and Mock Files
...
If using module bases as an external dependency with modulekit, this could cause compiler issues. We recommend removing the module bases dependency and only using modulekit.