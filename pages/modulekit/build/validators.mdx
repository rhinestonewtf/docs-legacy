...
This page only aims to cover domain 2 in the specific case of a Validator Module. For more information on the other two, read through the [Module](./module.mdx) page.
...
Additionally, the `ERC7579ValidatorBase` includes a function `_packValidationData(bool sigFailed, uint48 validUntil, uint48 validAfter)`, which packs the return data according to the ERC-4337 specifications on more complex return data. The schema of this return data is: `<20-byte> aggregatorOrSigFail`, `<6-byte> validUntil` and `<6-byte> validAfter`. This can be used to either use an aggregator or to ensure that a `UserOperation` can only be executed before `validUntil` or after `validAfter`. To easily format the return data to use this schema, use `_packValidationData`.
