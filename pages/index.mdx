...
Account Abstraction is a powerful tool for web3 builders. Smart contract accounts (smart accounts herein) allow for programmable validity and programmable execution, which has brought about heightened account security (e.g. multi-sig) and improved dapp experiences (e.g. session keys and batched transactions). However, access to this new programmable platform has been limited to the core teams working on smart accounts and their accompanying UI (the wallet).
